...
Интерфейс должен позволять просматривать, анализировать и визуализировать личные финансовые данные, получаемые из единой базы данных (PostgreSQL).
...
* Использовать общую базу данных (PostgreSQL, ранее expenses.db), которую использует Telegram-бот.
...
* Обеспечить работу UI на локальной машине ([http://localhost:8080](http://localhost:8080) или иной порт).
...
3. **Добавить кнопку с ссылкой на Web UI в Telegram-боте**
   ...
   * Для теста: [http://localhost:8080/?user\_id=123456](http://localhost:8080/?user_id=123456)
...
Все данные берутся из одной и той же базы (`PostgreSQL`, ранее `expenses.db`).
...
**Примерная ссылка:**
[http://localhost:8080/?user\_id=123456](http://localhost:8080/?user_id=123456)
