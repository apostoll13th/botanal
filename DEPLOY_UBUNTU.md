...
> ⚠️ Актуальная конфигурация использует `docker-compose` (см. `docker-compose.yml`) и PostgreSQL. Ниже приведена историческая инструкция для ручного развёртывания SQLite-версии и может быть использована только как справочная.
